...
1. **Scenario:** Consulta exitosa de ruta
   - **Given:** El Conductor tiene sesión activa y una ruta ha sido confirmada por el Despachador y está en estado 'Ruta Confirmada'.
...
2. **Scenario:** Inicio de tránsito por el conductor
   ...
   - **Then:** La ruta pasa a estado 'En Tránsito', se registra la fecha y hora de salida, el Módulo 1 es notificado para que cada paquete de la ruta cambie a estado 'En Tránsito'.

3. **Scenario:** Consulta sin ruta asignada
   - **Given:** El Conductor tiene sesión activa pero no tiene ninguna ruta en estado 'Ruta Confirmada' asignada a su dispositivo.
   - **When:** Consulta su ruta activa.
...
| ID | Requisito |
|---|---|
| **FR-021** | El sistema DEBE mostrar al Conductor su ruta asignada en estado 'Ruta Confirmada' con el orden de paradas, dirección y detalle de cada paquete. |
...
| **FR-024** | Al confirmar el inicio del tránsito, el sistema DEBE cambiar el estado de la ruta a 'En Tránsito', registrar la fecha y hora de salida y notificar al Módulo 1 para que cada paquete pase a estado 'En Tránsito'. |
...
| **SC-021** | Al confirmar el inicio del tránsito, el Módulo 1 recibe la notificación y actualiza el estado de todos los paquetes de la ruta a 'En Tránsito' en menos de 5 segundos. |
